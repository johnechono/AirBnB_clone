...
def parse(line):
    ''' it parses the line (string) from prompt  before the execution'''
    argv = line.split(' ')
    if len(argv) > 4:
        argv = argv[:4]
    if len(argv) != 4:
    ...
    # parses upate command (remove quotes)
    print(argv)
    if argv[3][0] in ['"', '"']:
        argv[3] = argv[3][1:]
    if argv[3][-1] in ['"', '"']:
...
def err_manager(line, argc):
    '''it  manages errors while parsing '''
    if not line:
...
def type_checker(value):
    '''
        it typecasts the value into an adequate type before
...
class HBNBCommand(cmd.Cmd):
    ''' it defines the HBNBCommand interpreter '''
    ...
    def do_EOF(self, line):
        '''
        Description:
        - quits the command interpreter on
            EOF marker (if no text in cmd line)
        - performs the forward delete if in between two
            characters or beginning of line.
        -> Usage: Ctrl + D
        '''
        return True
    ...
    def do_show(self, line):
        ''' it implements the show command '''
    ...
    def do_destroy(self, line):
        ''' it implements the destroy command '''
    ...
    def do_all(self, cls_name):
        ''' it implements the create command '''
        objects = storage.all().values()
        if not cls_name:
    ...
    def do_update(self, line):
        '''
        Description:
            it updates the instance attributes either through a key value pair
    ...
    def default(self, line):
        ''' it implements the default commands  '''
        objects = storage.all().values()
        argv = line.split('.', 1)
